...
describe('App', () => {
  ...
  it('renders product cards after load', async () => {
    vi.mocked(api.fetchProducts).mockResolvedValue({ source: 'db', data: mockProducts })
    render(<App />)
  ...
  it('shows POSTGRES source badge when source is db', async () => {
    vi.mocked(api.fetchProducts).mockResolvedValue({ source: 'db', data: mockProducts })
    render(<App />)
  ...
  it('shows REDIS CACHE source badge when source is cache', async () => {
    vi.mocked(api.fetchProducts).mockResolvedValue({ source: 'cache', data: mockProducts })
    render(<App />)
